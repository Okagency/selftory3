[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
$imgDir = "D:/dev/selftory3/output/MES구축가이드/images"

Add-Type -AssemblyName System.Drawing
function Resize-Jpeg {
 param($src, $dst, $maxW = 1000, $quality = 75)
 $img = [System.Drawing.Image]::FromFile($src)
 if ($img.Width -le $maxW) {
   $newWidth = $img.Width; $newHeight = $img.Height
 } else {
   $ratio = $maxW / $img.Width
   $newWidth = $maxW
   $newHeight = [int]($img.Height * $ratio)
 }
 $bmp = New-Object System.Drawing.Bitmap $newWidth, $newHeight
 $g = [System.Drawing.Graphics]::FromImage($bmp)
 $g.InterpolationMode = [System.Drawing.Drawing2D.InterpolationMode]::HighQualityBicubic
 $g.DrawImage($img, 0, 0, $newWidth, $newHeight)
 $encoder = [System.Drawing.Imaging.ImageCodecInfo]::GetImageEncoders() | Where-Object {$_.MimeType -eq "image/jpeg"}
 $params = New-Object System.Drawing.Imaging.EncoderParameters 1
 $params.Param[0] = New-Object System.Drawing.Imaging.EncoderParameter ([System.Drawing.Imaging.Encoder]::Quality, [long]$quality)
 $bmp.Save($dst, $encoder, $params)
 $g.Dispose(); $bmp.Dispose(); $img.Dispose()
}

# 6MB induction_melting 리사이즈
Resize-Jpeg "$imgDir/induction_melting.jpg" "$imgDir/induction_melting_resized.jpg" 1000 75
Write-Host ("induction_melting resized: " + (Get-Item "$imgDir/induction_melting_resized.jpg").Length)

# cnc_twinhead 800KB도 약간 리사이즈
Resize-Jpeg "$imgDir/cnc_twinhead.jpg" "$imgDir/cnc_twinhead_resized.jpg" 1000 75
Write-Host ("cnc_twinhead resized: " + (Get-Item "$imgDir/cnc_twinhead_resized.jpg").Length)

function B64 { param($path, $mime); $bytes = [System.IO.File]::ReadAllBytes($path); return "data:$mime;base64," + [Convert]::ToBase64String($bytes) }
function Fig { param($src, $caption); return '<figure><img src="' + $src + '" alt=""><figcaption>' + $caption + '</figcaption></figure>' }

# 주조 사진 13장
$cold_die = B64 "$imgDir/cold_chamber_die_casting.svg" "image/svg+xml"
$hot_die = B64 "$imgDir/hot_chamber_die_casting.svg" "image/svg+xml"
$hpdc_4000 = B64 "$imgDir/hpdc_4000t.jpg" "image/jpeg"
$buhler = B64 "$imgDir/buhler_diecasting.jpg" "image/jpeg"
$hpdc_plant = B64 "$imgDir/hpdc_bay_urse.jpg" "image/jpeg"
$engine_bare = B64 "$imgDir/engine_block_bare.jpg" "image/jpeg"
$engine_18xer = B64 "$imgDir/engine_block_18xer.jpg" "image/jpeg"
$induction_sh = B64 "$imgDir/induction_sheffield.jpg" "image/jpeg"
$cnc_cds = B64 "$imgDir/cnc_dynamics.jpg" "image/jpeg"
$northup = B64 "$imgDir/northup_furnace.jpg" "image/jpeg"
$cnc_th = B64 "$imgDir/cnc_twinhead_resized.jpg" "image/jpeg"
$wheel = B64 "$imgDir/alloy_wheel.jpg" "image/jpeg"
$melting = B64 "$imgDir/induction_melting_resized.jpg" "image/jpeg"

$casting_section = @"
<section id="ch1-images">
 <h2 class="part">주조 설비·도식·부품 사진
 <small>1·2장 본문 보조 자료 — Wikimedia Commons CC-BY-SA</small>
 </h2>

 <p>주조 핵심 설비·금형·자동차 부품 시각 자료. 본문 1·2장과 함께 본다. 13장 사진·도식.</p>

"@ + (Fig $induction_sh "사진 1.3a — 유도로 (Electric Induction Furnace · Sheffield). 전자기 유도로 알루미늄 합금 700~800도 용해. 자동차 주조 협력사 표준 용해 설비. 코일·도가니·LF·HF (50~200 kHz). (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $northup "사진 1.3b — Northup 유도로. 코어리스 유도로 초기 표준 설계. 알루미늄·구리·황동 용해. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $melting "사진 1.3c — 유도 가열 용해 진행 중. 용탕이 800도+에서 유지·교반·성분 보정. 작업자 보호복·고온 환경. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $cold_die "도식 1.5a — 콜드 챔버 다이캐스팅 머신 스키매틱. 알루미늄 합금 표준. 사출 슬리브에서 플런저가 용탕을 금형 캐비티로 700~1,000 bar 고압 주입. (Wikimedia · CC-BY-SA 3.0)") + "`r`n" + `
(Fig $hot_die "도식 1.5b — 핫 챔버 다이캐스팅 머신 스키매틱. 아연·마그네슘 합금. 융탕이 머신 안 보온로에 직접 들어있음. 가오리 펌프 사출. (Wikimedia · CC-BY-SA 3.0)") + "`r`n" + `
(Fig $hpdc_4000 "사진 1.5c — 4000톤 고압 다이캐스팅(HPDC) 머신. 미션 케이스·엔진 블록 등 대형 구조 부품 양산 설비. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $buhler "사진 1.5d — Bühler 다이캐스팅 머신. 스위스 Bühler는 글로벌 자동차 다이캐스팅 머신 1위. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $hpdc_plant "사진 1.5e — HPDC 공장 라인 (Bay-Urse Plant). 머신·금형·인출 로봇·후공정 일체화. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $cnc_cds "사진 1.7a — CNC 가공 머신 (CNC Dynamics CDS). 주조 후처리 — 홀·면·나사·정밀 가공. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $cnc_th "사진 1.7b — CNC TwinHead 가공 머신. 머시닝 센터의 2축 동시 가공 — 생산성·정밀도 향상. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $engine_bare "사진 2.1a — 자동차 엔진 블록 (B8444S Volvo). 알루미늄 합금 HPDC·CNC 가공 후 표면. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $engine_18xer "사진 2.1b — 18XER 엔진 블록. 자동차 엔진 블록의 머시닝 후 표면 디테일. (Wikimedia · CC-BY-SA)") + "`r`n" + `
(Fig $wheel "사진 2.1c — 알루미늄 알로이 휠 (Subaru JWL 마크). 저압 다이캐스팅(LPDC) 표준 부품. A356 합금·T6 열처리. (Wikimedia · CC-BY-SA)") + @"

</section>

"@

$casting_file = "D:/dev/selftory3/output/MES구축가이드/mes구축가이드-주조.html"
$content = [System.IO.File]::ReadAllText($casting_file, [System.Text.Encoding]::UTF8)
$pattern = '(?s)<section id="ch1-images">.*?</section>\s*'
$content = $content -replace $pattern, $casting_section
[System.IO.File]::WriteAllText($casting_file, $content, $utf8NoBom)
Write-Host "casting images: 13 photos"

Write-Host "Done"
