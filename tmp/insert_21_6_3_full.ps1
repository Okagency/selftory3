...
$v2Path = 'D:\dev\selftory\tmp\21_6_3_v2_deep.txt'
$v3AddonPath = 'D:\dev\selftory\tmp\21_6_3_v3_addon.txt'

$content = Get-Content -Path $bookPath -Raw -Encoding UTF8
$v2 = Get-Content -Path $v2Path -Raw -Encoding UTF8
$v3addon = Get-Content -Path $v3AddonPath -Raw -Encoding UTF8

# Combine v2 + v3 addon
$combined = $v2.TrimEnd() + "`r`n" + $v3addon.TrimEnd()
...
$newContent = $before + $combined + ' ' + $after
